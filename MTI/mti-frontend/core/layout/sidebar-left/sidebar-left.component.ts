...
import { Funcionalidade } from '../../../src/app/shared/enum/funcionalidade.enum';
...
/*    this.subscriptions.push(this.layoutStore.sidebarLeftMenu.subscribe(value => {
  ...
  montaMenu(): any[] {
    ...
    if (AuthStorage.usuarioTemPermissaoRole(Funcionalidade.MNU_DASHBOARD)) {
      menu.push({ label: 'MAIN NAVIGATION', separator: true });
      menu.push({ label: ' Dashboard', route: 'dashboard', iconClasses: 'fa fa-lg fa-dashboard' });
    }

    let adicionadoAdministracao = false;

    if (AuthStorage.usuarioTemPermissaoRole(Funcionalidade.MNU_USUARIO)) {
      menu.push({ label: 'ADMINISTRAÇÃO', separator: true });
      adicionadoAdministracao = true;
      menu.push({ label: ' Cadastro de Usuários', route: 'usuario', iconClasses: 'fa fa-lg fa-users text-green' });
    }

    // const subMenuLDAP = new Array();
    // subMenuLDAP.push({ label: 'Domínio', route: 'dominioLdap' });
    // subMenuLDAP.push({ label: 'Configuração', route: 'configuracaoLdap' });
    // menu.push({ label: ' LDAP', iconClasses: 'fa fa-lg fa-cogs', children: subMenuLDAP });

    let adicionadoGestaoAcesso = false;
    const subMenuGestaoAcesso = new Array();
    if (AuthStorage.usuarioTemPermissaoRole(Funcionalidade.MNU_PERFIL)) {

      if(!adicionadoAdministracao){
        menu.push({ label: 'ADMINISTRAÇÃO', separator: true });
        adicionadoAdministracao = true;
      }
      
      subMenuGestaoAcesso.push({ label: 'Perfil', route: 'perfil' });
      adicionadoGestaoAcesso = true
    }
    
    if (AuthStorage.usuarioTemPermissaoRole(Funcionalidade.MNU_FUNCIONALIDADE)) {
      ...
      subMenuGestaoAcesso.push({ label: 'Funcionalidades', route: 'funcionalidade' });
      adicionadoGestaoAcesso = true
    }    

    if(adicionadoGestaoAcesso){
      menu.push({ label: ' Gestão de Acesso', iconClasses: 'fa fa-lg fa-lock', children: subMenuGestaoAcesso });
    }
